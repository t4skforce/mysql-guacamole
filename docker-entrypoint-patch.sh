...
_main() {
	...
	if [ "$1" = 'mariadbd' ] || [ "$1" = 'mysqld' ] && ! _mysql_want_help "$@"; then
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
			...
			# check dir permissions to reduce likelihood of half-updated database
			ls /docker-entrypoint-upgrade.d/ > /dev/null
			...
			echo
			mysql_note "MariaDB update process done. Ready for start up."
			echo
		fi
	fi
	exec "$@"
